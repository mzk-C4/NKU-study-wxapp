...
> 日期：2026-08-22
>
> 状态：阶段一/二、五篇本地published、确定性AI最小闭环与30题自动化验收已完成；39项全量内容批量整理提示词已生成、尚未执行，完成并批准后再继续本地契约和AI检索扩展
>
...
- 已完成：SRC-001、SRC-002、SRC-003、SRC-004、SRC-005；
- 核心完成、附件后置：SRC-006辅修通知、SRC-007微专业通知；
- SRC-005已核验29份转专业原件与29份非空Markdown，父来源状态为`ready`；学院子来源只在进入个性化指南或问答时按需编号；
- `Documents/SOURCE_MANIFEST.md`已登记SRC-001至SRC-007；SRC-006/007的细分附件保持后置，不阻塞当前本地闭环；
...
- 阶段一提示词：[`LEARNING_COMPASS_STAGE1_CONTENT_DISTILLATION_PROMPT.md`](./LEARNING_COMPASS_STAGE1_CONTENT_DISTILLATION_PROMPT.md)，只产出五篇`draft`、内容索引、backlog和审查报告，不修改代码；
- 阶段二提示词：[`LEARNING_COMPASS_STAGE2_LOCAL_KB_PROMPT.md`](./LEARNING_COMPASS_STAGE2_LOCAL_KB_PROMPT.md)，只有阶段一经产品负责人明确批准后才能执行；
- 两步推荐在同一个新聊天框先后运行，以保留来源和审查上下文；
- 年级、学院、专业、时间不作为知识条目必填元数据；只有原文明示且理解答案确有必要时，才写入正文使用提醒；
- 阶段二只实现本地解析、校验、状态隔离、引用映射和指南/搜索投影，不实现AI、不修改生产契约或生产数据。

### 1.15 本地published与AI最小闭环验收

- 五篇经产品负责人审核的指南已获授权改为仅本地`published`，不代表生产发布；
- 生成版本为`851f17ad3663c9801d32a89c`，五分类各1篇，列表、详情`sections/sources`和搜索投影已跑通；
- 本地确定性assistant已验证只检索published、引用返回、无依据/冲突拒答、provider重试1次与`AI_UNAVAILABLE`降级；
- 30题本地回归为30/30，其中25题命中引用、5题拒答；该结果是确定性fixture回归，不代表真实模型、原文链接可达性或生产质量已验收；
- 本地路由默认关闭且只允许loopback显式启用；正式`docs/API.md`、小程序在线AI调用和生产服务未改变。
...
- 首页、AI已有回答和AI断网恢复视觉均已获产品负责人批准；详情页视觉仍待生成与批准。

### 4.1.2 AI 网络断开恢复页实现

- 产品负责人批准 `图片和附件/learning-compass-ai-network-offline-approved-20260823.png`，要求网络断开时按该图一比一复刻；
- 新增 `pages/guide-assistant/index`，网络断开后保留当前问题，并提供复制、就地编辑、检查网络、浏览知识库、普通搜索和底部输入；
- 产品负责人后续批准 `图片和附件/learning-compass-ai-answer-approved-20260823.png` 作为已有回答状态结构权威；同一页面增加本地回答预览、引用、来源卡片、回答操作和轮次说明；
- 已回答与断网状态共享顶部“对话记录”“新开话题”原生按钮；最近问题在本机保存，新开话题不删除历史；“对话记录”已改为左侧抽屉，支持搜索、图钉置顶/取消置顶和“置顶、今天、昨天、7天内、30天内”分组，不含个人资料区；长按会话在条目下方展开自定义“重命名/置顶或取消置顶/删除”操作卡，删除仍需二次确认；
- 指南首页在线点击AI入口仍提示建设中，断网时进入恢复页；页面没有调用未开放AI端点；
- 为便于产品负责人验收，`develop` 环境在线点击会以固定示例问题打开 `preview=answer`；断网视觉仍可通过 `preview=network-error` 单独预览；trial/release不启用回答预览，仍保持在线建设中、断网恢复页的真实逻辑；
- 问题和输入草稿只保存在本机；历史会话按每条记录更新时间保留30天，读取失败不阻塞恢复入口；
- 附件图标按批准参考修正为竖向双层回形针；仍为不可用占位，点击只提示“暂不支持附件”，不会打开选择器或上传；
- 网络恢复后只确认连接恢复，不伪造AI回答；
- 自动化覆盖回答预览、历史抽屉、搜索、独立会话标题、重命名、图钉置顶、抽屉内长按菜单、确认删除、30天淘汰、新开话题、连续输入焦点、回答复制、原文跳转占位、反馈、断网发送、监听恢复、手动重试、就地编辑、两个降级入口、路由编码和零AI请求；
- 微信开发者工具自动打开仍因安装目录未找到而阻塞，像素级视觉验收需用户手动完成。

来源打开按现有小程序能力分级处理：`resources.nkustudy.top` 等已配置域名的文件可下载后用 `wx.openDocument` 打开；已配置的站内来源可进入受控来源页；未配置的外部 HTTPS 页面不在客户端任意拼接 `web-view`。AI回答页的“查看原文”只作为未来定位跳转入口，当前提示“原文跳转正在建设中”，不再使用复制链接降级。若首批正式文件要求直接打开外部页面，必须先由生产 owner 配置合法域名或提供受控公开镜像。

AI回答来源卡按批准图采用两列结构：左侧固定文档图标，右侧标题占满内容列；文号、条款和固定短宽“查看原文”位于下方详情行。不得使用“图标＋标题＋按钮”三列同排，避免窄屏把中文标题压成逐字竖排。
...
当前状态：首期最小材料包已完成；SRC-006/007细分附件和自修冲突按已确认边界保持后置/冻结。

产出：使用材料审查提示词生成审查报告，并完成第 1.5 节盘点表。
...
当前状态：五篇指南的本地最小闭环已完成；长期`draft → review → published`运营界面与生产流程未实现。
...
当前状态：五分类列表、详情和搜索投影已完成；小程序真实原文打开、正式多章节/多来源契约和生产内容仍未完成。
...
当前状态：确定性检索、引用、拒答、重试1次和503降级已完成；30秒超时、每日20次限制、滞用控制和真实provider未实现。
...
当前状态：指南首页、AI已回答和断网恢复的本地页面已实现；详情页批准图、窄屏/大字体、微信开发者工具和真机验收未完成。

产出：依据产品负责人批准图片复刻指南首页、AI已有回答和AI断网恢复状态；详情页等待后续批准图。

完成条件：

- 页面结构和Style DNA来自对应批准图；
- 本地视觉预览不调用未开放AI端点，不改变生产API；
- 微信开发者工具、窄屏、大字体和真机完成逐页人工验收；
- 页面实现不改变已经冻结的接口与内容规则。
...
当前状态：首批30题确定性自动化回归已完成；多人盲审、原文链接实际可达、后端owner契约评审和生产验收未完成。
...
5. AI页面本地预览和断网恢复实现已存在，但正式问答端点、真实模型回答和生产证据仍不存在；像素级、窄屏、大字体和真机验收仍待用户执行。
6. 当前只有5篇指南进入本地published；39项backlog仍是全量内容建设池，不应把五篇样例和30题回归理解为全部内容已完成。
...
- [x] 金融学院2026版PDF已在项目子文件夹中到位，截图中的29份学院转专业附件齐全；
- [x] 用户完成SRC-003、SRC-004、SRC-006、SRC-007核心PDF和Markdown；
- [x] 建立 `Documents/SOURCE_MANIFEST.md` 第一版，关联原件、Markdown、官方链接和状态；
- [x] 生成阶段一内容沉淀提示词和阶段二本地知识库提示词，并设置人工审核门槛；
- [x] 用户在新聊天框执行阶段一并交回五篇草稿、backlog和审查报告；
- [x] 产品负责人复核3项修正并明确批准阶段一；
- [x] 阶段二建立本地解析、来源校验、状态隔离和指南/搜索投影闭环；
- [x] 阶段一8个交付文件已回交并完成首轮产品审核；
- [x] 按 [`LEARNING_COMPASS_STAGE1_PRODUCT_REVIEW.md`](./LEARNING_COMPASS_STAGE1_PRODUCT_REVIEW.md) 完成期中退课、微专业和国际小学期3项修正；
- [x] 修正结果复核通过，产品负责人明确批准阶段一；
- [x] SRC-005完成29份原件与29份Markdown，SOURCE_MANIFEST父来源改为ready；
- [x] 生成本地published、AI最小闭环和30题验收合并提示词；
- [x] 用户在完成阶段二的同一聊天框执行 [`LEARNING_COMPASS_LOCAL_PUBLISH_AI_EVAL_PROMPT.md`](./LEARNING_COMPASS_LOCAL_PUBLISH_AI_EVAL_PROMPT.md)；
- [x] 首期核心源材料已进入五分类和指南整理，SRC-005也已完成；
- [ ] 人工抽查学生手册候选条目的原文定位、适用范围和时效；
- [x] 在本地建立知识源、状态隔离、发布和引用样例；
- [x] 本地跑通拟议问答形状、拒答、重试和503降级；
- [ ] 在正式后端实现30秒超时、每日20次限制、滞用控制和真实provider；
- [ ] 后端 owner 评审独立接口需求文档；
...
- [x] 产品负责人批准AI已有回答图片，并按批准图复刻本地`preview=answer`状态；
- [x] 在AI已有回答和断网恢复状态补齐“对话记录”“新开话题”文字、图标及本机行为；
- [x] 将对话记录由底部菜单改为左侧抽屉，补齐搜索、置顶、30天时间分组并移除个人资料区；
- [x] 将星标置顶替换为图钉，补齐长按置顶/删除和删除二次确认；
- [x] 将长按原生菜单替换为条目下方自定义三项操作卡，并增加不改写原问题的会话重命名；
- [x] 修复底部输入框输入一个字符后失焦，并按参考图重绘附件回形针；
- [x] 将AI来源操作明确为未来原文定位跳转；资料库路由未完成时只提示建设中，不再复制链接；
- [x] 按批准图修正AI来源卡窄屏布局，标题独占内容列，查看原文按钮固定在详情行右下角；
- [x] 断网恢复批准图已实现；当前新增共享顶部操作但不改变其恢复语义；
- [ ] 实现或最终出图时把首页首条指南更新时间由误写的 `2025-08-17` 修正为 `2026-08-17`；
- [x] 完成首批30题确定性自动化回归与评分脚本；
- [ ] 由团队扩充盲审问题、多轮问法和真实模型评测。
