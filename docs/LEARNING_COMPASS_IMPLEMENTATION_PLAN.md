...
> 日期：2026-08-22
>
> 状态：阶段一/二、五篇本地published、确定性AI最小闭环与30题自动化验收已完成；正式后端契约、真实provider、超时/限流、全量内容和生产验收待继续
>
...
- 已回答与断网状态共享顶部“对话记录”“新开话题”原生按钮；最近问题在本机保存，新开话题不删除历史；“对话记录”已改为左侧抽屉，支持搜索、置顶/取消置顶和“置顶、今天、昨天、7天内、30天内”分组，不含个人资料区；
...
- 附件图标为不可用占位，点击只提示“暂不支持附件”，不会打开选择器或上传；
- 网络恢复后只确认连接恢复，不伪造AI回答；
- 自动化覆盖回答预览、历史抽屉、搜索、置顶、30天淘汰、新开话题、回答复制、原文跳转占位、反馈、断网发送、监听恢复、手动重试、就地编辑、两个降级入口、路由编码和零AI请求；
