...
Page({
  data: {
    ...
    mine: false, filterStatus: 'all', statusOptions: [
      { value: 'all', label: '全部' },
  ...
  async submit() {
    const { title, content, contact, submitting } = this.data
    ...
    try {
      const res = await submitFeedback({ title: title.trim(), content: content.trim(), contact: contact.trim() })
